...
class GitStackApp {
  ...
  getFileIcon(filename, type) {
    ...
    const icons = {
      ...
      dockerfile:
        '<i class="fab fa-docker" style="color: var(--ctp-blue);"></i>',
      env: '<i class="fas fa-key" style="color: var(--ctp-yellow);"></i>',
      gitignore:
        '<i class="fab fa-git-alt" style="color: var(--ctp-red);"></i>',
    };

    return (
      icons[extension] ||
      '<i class="fas fa-file" style="color: var(--ctp-overlay0);"></i>'
    );
  }
  ...
  syntaxHighlight(code, extension) {
    ...
    const patterns = {
      ...
      string: /(".*?"|'.*?'|`[\s\S]*?`)/g,
      keyword:
        /\b(const|let|var|function|class|if|else|for|while|return|import|export|from|async|await|try|catch|throw|new|this|true|false|null|undefined)\b/g,
  ...
  bindSettingsEvents() {
    ...
    document
      .getElementById("setting-line-numbers")
      ?.addEventListener("change", (e) => {
  ...
  async loadRecentActivity() {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.github.com/users/${this.currentUser.username}/events/public?per_page=10`,
        {
          headers: {
            Authorization: `token ${this.currentUser.githubToken}`,
            Accept: "application/vnd.github.v3+json",
